...
class App {
  ...
  private renderCategoryPills(): void {
    ...
    CATEGORIES.forEach((cat) => {
      const count =
        cat.id === 'all'
          ? CONTROLS_DATA.length
          : CONTROLS_DATA.filter((item) => item.category === cat.id).length;
  ...
  private getFilteredControls(): ControlItem[] {
    return CONTROLS_DATA.filter((item) => {
  ...
  private renderCards(): void {
    ...
    const items = this.getFilteredControls();

    if (this.resultsCount) {
      this.resultsCount.textContent = `Mostrando ${items.length} de ${CONTROLS_DATA.length} controles`;
    }
    ...
    items.forEach((item) => {
      ...
      this.controlsGrid.appendChild(card);

      // Render live interactive stage
